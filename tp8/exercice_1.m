...
while nb_points_delta_D > 0
    ...
    r = randi(nb_points_delta_D); 
    [p_x, p_y] = ind2sub(size(delta_D),indices_delta_D(r));
    
	 
	% Recherche du pixel q_chapeau :
	[existe_q,bornes_V_p,bornes_V_q_chapeau] = d_min(p_x,p_y,u,D,t,T)

	% S'il existe au moins un pixel q eligible :
	if existe_q

		% Rapiecage et mise a jour de D :
		[u_nouv,D_nouv] = rapiecage_1(bornes_V_p,bornes_V_q_chapeau,u,D);
        	u = u_nouv; 
            D = D_nouv; 
