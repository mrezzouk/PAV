...
% exercice_2_bis.m : Debruitage avec modele d’inpainting par variation totale
%--------------------------------------------------------------------------
...
for i=1:nb_lignes
    for j=1:nb_colonnes
        if (u0(i,j,1)>200) &&  (u0(i,j,2)>200) && (u0(i,j,3)<50)
            u_mask(i,j)=255;
        end
    end
end

W_prive_D = ones(size(u_mask)) - u_mask./255;
